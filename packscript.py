        ...
        for namespace in namespaces:
            ...
            for name, content in func_files.items():
                ...
                with open(path, 'w') as w:
                    w.write(f'# Generated by PackScript {__version__}{modified_by and f" modified by: {modified_by}"}\n'
                            + '\n'.join(content) + '\n')
...
def gen_template(*, name: str, description: str, pack_format: int, output: str, namespace: str, **_):
    ...
    description = description or input(f'Description (Datapack {name!r} for version {v}): ') or \
        f'Datapack {name!r} for version {v}'
